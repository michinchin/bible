...
typedef ColorPositionChanged<TPosition> = Function(TPosition position, Color color);
...
class ColorPickerParameters {
  ...
  static const defaultParameters = ColorPickerParameters(
      ...
      colorHexInputDecoration:
          InputDecoration(border: OutlineInputBorder(), labelText: 'Hex', prefix: Text('#')),
  ...
  ColorPickerParameters copyWith(
    ...
    bool withAlpha, // ignore: avoid_positional_boolean_parameters
    ...
    double comboColorContainerHeight,
      ...
      ColorPickerParameters(
        ...
        colorHexInputDecoration: colorHexInputDecoration ?? this.colorHexInputDecoration,
        ...
        titleDecoratorBuilder: titleDecoratorBuilder ?? this.titleDecoratorBuilder,
        comboPopupSize: comboPopupSize ?? this.comboPopupSize,
        comboColorContainerHeight: comboColorContainerHeight ?? this.comboColorContainerHeight,
      );
  ...
  static Widget buildDefaultTitleDecorator(
    ...
    final decoration = InputDecoration(labelText: title, border: const OutlineInputBorder())
    ...
    return Stack(
      children: [
        Material(
            borderRadius: (decoration.enabledBorder as OutlineInputBorder)?.borderRadius,
            child: child),
        Positioned.fill(
          child: IgnorePointer(
...
class ColorPickerContext extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final def =
        parentData == null ? ColorPickerParameters.defaultParameters : parentData.parameters;
    final my = parameters;
    final merged = ColorPickerParameters(
      ...
      colorHexInputDecoration: my.colorHexInputDecoration ?? def.colorHexInputDecoration,
      ...
      titleDecoratorBuilder: my.titleDecoratorBuilder ?? def.titleDecoratorBuilder,
      comboPopupSize: my.comboPopupSize ?? def.comboPopupSize,
      comboColorContainerHeight: my.comboColorContainerHeight ?? def.comboColorContainerHeight,
    );
...
class _ColorPickerComboState extends State<ColorPickerCombo> implements ComboController {
  ...
  Widget build(BuildContext context) {
    ...
    final parameters =
        ColorPickerContext.of(context)?.parameters ?? ColorPickerParameters.defaultParameters;
      ...
      parameters: ComboParameters(
        ...
        childDecoratorBuilder: (context, comboParameters, controller, child) =>
            parameters.titleDecoratorBuilder(
                context, parameters, comboParameters, controller, widget.title, child),
      ),
      child: Combo(
        key: _comboKey,
        child: ListTile(
...
class _ColorHexState extends State<ColorHex> {
  ...
  static Color _getColor(Color color, bool withAlpha) => withAlpha ? color : color.withAlpha(0xff);

  static String _getColorHex(Color color, bool withAlpha) {
    final s = (withAlpha ? color.value : color.value - (color.alpha << 24)).toRadixString(16);
    return Iterable<int>.generate((withAlpha ? 8 : 6) - s.length).map((e) => '0').join() + s;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return TextField(
      ...
      decoration: parameters.colorHexInputDecoration.copyWith(errorText: _errorText),
    );
  }
...
class _AlphaPainter extends CustomPainter {
  ...
  static void paintAlpha(Canvas canvas, Rect rect, Color color, double size, [int initial = 0]) {
    final paint = Paint()..color = color;
    for (var i = initial; i * size < rect.width; i++) {
      for (var j = 0; j * size < rect.height; j++) {
        if (i % 2 != j % 2) continue;
        canvas.drawRect(
            Rect.fromLTWH(rect.left + i * size, rect.top + j * size, size, size), paint);
      }
    }
  }
  ...
  void paint(Canvas canvas, Size size) =>
      paintAlpha(canvas, Rect.fromLTWH(0, 0, size.width, size.height), alphaColor, alphaRectSize);
...
class _PaletteState extends State<Palette> {
  ...
  void _updatePosition(Offset position, double width, double height) {
    final x = ((width - position.dx) / width).clamp(0.0, 1.0).toDouble();
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return GestureDetector(
        onPanDown: (details) => _updatePosition(details.localPosition, width, height),
        onPanUpdate: (details) => _updatePosition(details.localPosition, width, height),
        child: Container(
          clipBehavior: Clip.antiAlias,
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(parameters.alphaRectSize)),
          child: Stack(
            children: [
              SizedBox(
                width: width,
                height: height,
                child: CustomPaint(painter: _PalettePainter(widget.rainbowColor)),
              ),
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: parameters.paletteCursorColor, width: parameters.paletteCursorWidth),
                  ),
                ),
              ),
            ],
...
class _RainbowSliderState extends State<RainbowSlider> {
  _RainbowSliderState(this._position) : _color = RainbowSlider.getColor(_position);
  double _position;
  Color _color;
...
// ignore: prefer_mixin
class _RainbowSliderTrackShape extends SliderTrackShape with BaseSliderTrackShape {
...
class _AlphaSliderState extends State<AlphaSlider> {
  ...
  Widget build(BuildContext context) {
    ...
    return SliderTheme(
      data: SliderThemeData(
          trackShape: _AlphaSliderTrackShape(color: color, alphaColor: parameters.alphaColor),
          trackHeight: parameters.trackHeight,
          thumbColor: color,
          overlayColor: color.withOpacity(0.33)),
      child: Slider(
...
// ignore: prefer_mixin
class _AlphaSliderTrackShape extends SliderTrackShape with BaseSliderTrackShape {
  const _AlphaSliderTrackShape({@required this.color, @required this.alphaColor});
  ...
  void paint(
    ...
    canvas.drawRRect(
        RRect.fromRectAndCorners(Rect.fromLTWH(rect.left, rect.top, size, size), topLeft: radius),
        paint);
    _AlphaPainter.paintAlpha(canvas,
        Rect.fromLTWH(rect.left, rect.top, rect.width - size, rect.height), alphaColor, size, 1);
    canvas.drawRRect(
        RRect.fromRectAndCorners(rect,
            topLeft: radius, topRight: radius, bottomLeft: radius, bottomRight: radius),
        Paint()
          ..shader = LinearGradient(colors: [color.withOpacity(0.0), color]).createShader(rect));
  }
}
